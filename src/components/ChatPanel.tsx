 ...
 * neighbors) so the LLM answers from the user's graph, not from
 * general training data.
 *
 * History is ephemeral per-session. Switching nodes clears it —
 * questions about a different node shouldn't leak the prior node's
 * reasoning trail. Persistent chat-as-node comes later.
 */
export function ChatPanel({ nodeId }: Props) {
  ...
  // The assistant message currently being streamed. Tracked in a ref so
  // incoming chunks append to the latest value even across event bursts.
  const streamingMsg = useRef<string>("");

  // Clear transcript whenever the user navigates to a different node.
  useEffect(() => {
    setHistory([]);
    setInput("");
    setError(null);
  ...
  // Subscribe to chat-* events once; the listeners read current state
  // via the streamingMsg ref and the history setter.
  useEffect(() => {
  ...
  // Autoscroll to bottom as tokens arrive.
  useEffect(() => {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setStreaming(true);
    try {
      // Send history WITHOUT the fresh user turn — the backend appends
      // it alongside the system prompt and context block.
      ...
      <div className="chat-transcript" ref={scrollRef}>
        {history.length === 0 && !streaming && (
          <p className="muted chat-hint">
            Ask a question about this node. The assistant sees its content,
            backlinks, outgoing links, and top neighbors — your graph, not
            the open web.
          </p>
        )}
        {history.map((m, i) => (
